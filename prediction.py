...
df["ma_5"] = df.groupby("company_code")["Close"].rolling(5).mean().reset_index(0, drop=True)
df["ma_10"] = df.groupby("company_code")["Close"].rolling(10).mean().reset_index(0, drop=True)
df["volatility"] = df.groupby("company_code")["Close"].rolling(10).std().reset_index(0, drop=True)

df.dropna(inplace=True)

# Target = next time-step close
...
features = ["Close", "Volume", "return", "ma_5", "ma_10", "volatility"]
